...
function Home() {
    ...
    //     const response = await axios.get(BASE_URL + '/api/v1/home/banner-details');
    //     let mainImages = [];
    //     if (response.status === 200) {
    //         response.data.Details.forEach(function(i) {mainImages.push(i.ImageUrl)});
    //     } else {
    //         mainImages = ['url(shutterstock_407632243.jpg)', 'url(shutterstock_696636346.jpg)'];
    //     }
    //     setIsLoaded(true);
    //     return mainImages;
    // };
    // mainImages = getImages();

    // The image URLs are returned by the API, but they are not rendered
    mainImages = ['shutterstock_407632243.jpg', 'shutterstock_696636346.jpg'];
